...
export default function GridView({ directorio, edit }: ViewProps) {
    ...
    return (
        <motion.div
            ...
            className='h-[20rem] sm:h-[30rem] rounded-md shadow-lg bg-zinc-800 border border-neutral-950'>
            <div className='flex items-center justify-center w-full h-[50%]'>
                <div className='flex h-full w-full rounded-t-md relative overflow-hidden'>
                    {directorio.fotos.map((foto, index) => (
                        <motion.div
                            ...
                            <Image className='w-full h-full object-cover rounded-t-md pointer-events-none' src={foto} alt={`imagen-${index}`} width={20} height={20} />
                        </motion.div>
                    ))}
                    {
                        ...
                        <>
                            <motion.svg
                                ...
                                className="size-7 absolute right-0 self-center mr-1 mix-blend-difference cursor-pointer">
                            ...
                            <motion.svg
                                ...
                                onClick={handleLeft}
                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                className="size-7 absolute left-0 self-center ml-1 mix-blend-difference cursor-pointer">
            ...
            <div className='h-[50%] w-full p-3'>
                <div className='flex justify-between items-center w-full bg-stone-700 px-2'>
                    <h3 className='font-dosis font-medium uppercase tracking-wider text-base lg:text-2xl my-1 overflow-hidden w-[50%] lg:w-[65%]'>{directorio.nombre}</h3>
                    <motion.a
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.95 }}
                        className='flex justify-center rounded-sm items-center font-bold uppercase text-xs lg:text-sm font-dosis tracking-widest w-[50%] lg:w-[30%] h-full p-1 bg-slate-900 hover:bg-slate-800' href={directorio.url}>Sitio Web</motion.a>

                </div>
                <div className='relative mt-2 lg:mt-10 h-[70%] lg:h-[60%] font-sometype italic overflow-hidden'>
                    ...
                    <div className='flex justify-between absolute lg:bottom-0 w-full bg-stone-900'>
                        ...
                        {edit === true &&
                            <div className='w-[23%]'>
                                ...
                                <button onClick={() => dispatch({ type: 'delete-directoy', payload: { id: directorio.id } })} type='button' className='w-[50%] h-full bg-stone-900 border border-stone-950 rounded-sm hover:bg-gradient-to-br from-stone-800 text-red-500'>
                                    <motion.div whileHover={{ scale: 1.3 }} whileTap={{ scale: 0.8 }} className='flex justify-center items-center h-full w-full'>
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-5 lg:size-6">
